...
function createCard(product) {
  ...
  // URL por defecto
  let href = "#";
  if (category.toLowerCase() === "torso" && type.toLowerCase() === "blazer") {
    href = "../../views/torso/blazer.html";
  }
...
async function loadProductCards() {
  try {
    ...
    const productosFiltrados = products.filter((product) => {
      ...
      return category === "torso" && type === "blazer";
    });

    if (productosFiltrados.length === 0) {
      container.innerHTML = `<p class="text-warning">No hay productos en categoría TORSO y tipo BLAZER.</p>`;
      return;
    }
