...
export const addStateUser = async (req, res) => {
    try {
        const { State_users_name } = req.body;
        if (!State_users_name)  {
            return res.status(400).json({ error: "Missing required fields" });
        }
        let sqlQuery = "INSERT INTO state_user (State_user_name) VALUES (?)";
        const [result] = await connect.query(sqlQuery, [ State_users_name ]);
        res.status(201).json({
            data: [{ id: result.insertId, State_users_name }],
            status: 201
        });
    } catch (error) {
...
export const updateStateUser = async (req, res) => {
    try {
        ...
        const [result] = await connect.query(sqlQuery, [State_users_name, req.params.id ]);
        if (result.affectedRows === 0) return res.status(404).json({ error: "state_user not found"});
        res.status(200).json({
            data: [{ State_users_name }],
            status: 200,
