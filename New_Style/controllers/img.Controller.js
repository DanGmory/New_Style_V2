...
const pathImage = '../public/assets/imgs';
...
export const uploadFile = [
  ...
  async (req, res) => {
    try {
      ...
      const imageUrl = `/public/assets/imgs/${req.file.filename}`;
...
export const updateImage = [
  ...
  async (req, res) => {
    try {
      ...
      // Si hay nuevo archivo, actualiza y elimina el anterior
      if (req.file) {
        const fullPath = path.join(__dirname, '..', prevImg.Image_url);
        if (fs.existsSync(fullPath)) fs.unlinkSync(fullPath);

        imageUrl = `/public/assets/imgs/${req.file.filename}`;
      }
