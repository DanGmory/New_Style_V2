...
function createCard(product) {
  ...
  // URL por defecto
  let href = "#";
  if (category.toLowerCase() === "torso" && type.toLowerCase() === "gaban") {
...
async function loadProductCards() {
  try {
    ...
    const productosFiltrados = products.filter(product => {
      ...
      return category === "torso" && type === "gaban";
    });

    if (productosFiltrados.length === 0) {
      container.innerHTML = `<p class="text-warning">No hay productos en categoría TORSO y tipo GABAN.</p>`;
      return;
    }
