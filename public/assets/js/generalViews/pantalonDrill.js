...
function createCard(product) {
  ...
  // URL por defecto
  let href = "#";
  if (category.toLowerCase() === "pantalon" && type.toLowerCase() === "drill") {
    href = "../../views/pantalones/PantalonDrill.html";
  }
...
async function loadProductCards() {
  try {
    ...
    const productosFiltrados = products.filter((product) => {
      ...
      return category === "pantalon" && type === "drill";
    });

    if (productosFiltrados.length === 0) {
      container.innerHTML = `<p class="text-warning">No hay productos en categoría PANTALON y tipo DRILL.</p>`;
      return;
    }
