...
function createCard(product) {
  ...
  const name = product.Product_name ? product.Product_name.trim().toUpperCase() : "SIN NOMBRE";
  const category = product.Product_category ? product.Product_category.trim().toUpperCase() : "SIN CATEGORIA";
  const type = product.Type_product_name ? product.Type_product_name.trim().toUpperCase() : "SIN TIPO";
  ...
  // URL por defecto
  let href = "#";
  if (category.toLowerCase() === "torso" && type.toLowerCase() === "abrigo formal") {
    href = "../../views/torso/gaban.html";
  }
  ...
  const tooltip = isLoggedIn() ? "" : 'title="Inicia sesión para agregar productos"';
...
async function loadProductCards() {
  try {
    ...
    const productosFiltrados = products.filter(product => {
      ...
      return category === "torso" && type === "abrigo formal";
    });

    if (productosFiltrados.length === 0) {
      container.innerHTML = `<p class="text-warning">No hay productos en categoría TORSO y tipo ABRIGO FORMAL.</p>`;
      return;
    }
    ...
    document.getElementById("productCards").innerHTML = `<p class="text-danger">No se pudieron cargar los productos.</p>`;
  }
}
